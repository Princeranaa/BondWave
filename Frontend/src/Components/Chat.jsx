import React, { useRef, useEffect } from "react";
import { useParams, Link } from "react-router-dom";
import { ArrowLeft } from "lucide-react"; // optional icon (install if needed)

function Chat() {
  const { targetUserId } = useParams();
  console.log("targetUserId", targetUserId)

  return (

    <div className="min-h-screen  bg-gradient-to-br from-purple-900 via-black to-gray-900 flex justify-center  p-4">
      
      {/* Floating Chat Card */}
      <div className="w-screen max-w-4xl h-[80vh] rounded-2xl bg-base-300/60 backdrop-blur-xl shadow-2xl border border-white/10 flex flex-col overflow-hidden animate-fadeIn">
        
        {/* Header */}
        <div className="p-4 bg-gradient-to-r from-purple-900 via-black to-gray-900  text-white flex items-center gap-3 shadow-md">
          <Link to="/connections">
            <button className="btn btn-circle btn-sm bg-white/20 border-none hover:bg-white/30">
              <ArrowLeft size={18} />
            </button>
          </Link>

          <div className="avatar">
            <div className="w-12 rounded-full ring-2 ring-white/40">
              <img src="https://images.unsplash.com/photo-1502685104226-ee32379fefbe" />
            </div>
          </div>

          <div>
            <h2 className="text-xl font-bold">Person Name</h2>
            <p className="text-sm opacity-80">Online</p>
          </div>
        </div>

        {/* Messages Area */}
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          
          {/* Left message */}
          <div className="flex items-start gap-2">
            <div className="avatar">
              <div className="w-8 rounded-full">
                <img src="https://images.unsplash.com/photo-1502685104226-ee32379fefbe" />
              </div>
            </div>

            <div className="p-3 bg-base-200 rounded-xl break-words whitespace-pre-wrap max-w-[70%] shadow-md">
              <p>Hey! 👋 I saw your profile and wanted to say hi.</p>
              <span className="text-[10px] opacity-60 block mt-1">2:21 PM</span>
            </div>
          </div>

          {/* Right message */}
          <div className="flex items-end justify-end gap-2">
            <div className="p-3 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-xl max-w-[70%] shadow-lg">
              <p>Hey! Happy to connect 😊</p>
              <span className="text-[10px] opacity-80 block mt-1 text-right">2:22 PM</span>
            </div>

            <div className="avatar">
              <div className="w-8 rounded-full">
                <img src="https://i.pravatar.cc/150?img=12" />
              </div>
            </div>
          </div>

          {/* More sample messages */}
          {/* <div className="flex items-start gap-2">
            <div className="avatar">
              <div className="w-8 rounded-full">
                <img src="https://images.unsplash.com/photo-1502685104226-ee32379fefbe" />
              </div>
            </div>

            <div className="p-3 bg-base-200 rounded-xl max-w-[70%] shadow-md">
              <p>What are you up to today?</p>
              <span className="text-[10px] opacity-60 block mt-1">2:23 PM</span>
            </div>
          </div> */}
        </div>

        {/* Input Bar */}
        <div className="p-4 bg-base-200 flex items-center gap-3 border-t border-white/10">
          <input
            type="text"
            placeholder="Type a message..."
            className="input input-bordered w-full bg-base-100 rounded-full shadow-md"
          />
          <button className="btn btn-primary rounded-full px-6 shadow-lg hover:scale-105 transition">
            Send
          </button>
        </div>
      </div>
    </div>
  );
}

export default Chat;
